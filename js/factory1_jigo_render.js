...
(function () {
    ...
    /* ────────────────────────────────────────────────────────────
       승계(carry-forward)
       ────────────────────────────────────────────────────────────
       48.8g 처럼 값이 한 달에 한 번만 바뀌는 열은 매일 적지 않습니다.
       비어 있는 날은 직전에 적어 둔 값을 그대로 이어서 보여주고, 그 칸은
       '오늘 실제로 센 값'과 구분되게 옅게 그립니다.

       ※ 이 함수는 화면용 계산일 뿐입니다. DB 에 어떻게 담을지는 아직
         미정이고, 어느 쪽으로 정해지든 여기 로직은 그대로 씁니다.
       ──────────────────────────────────────────────────────────── */
    ...
    /* ────────────────────────────────────────────────────────────
       화면 확인용 임시 숫자  ※ DB 를 붙이면 이 블록을 통째로 지웁니다
       ────────────────────────────────────────────────────────────
       같은 (층·날짜·품목)이면 언제나 같은 숫자가 나옵니다. 스크롤할 때마다
       숫자가 달라지면 열 너비도 자릿수도 확인할 수 없기 때문입니다.
       ──────────────────────────────────────────────────────────── */
    ...
    function sampleRoll(floorKey, dateStr, c) {
        const absent = App.SAMPLE_ABSENT[floorKey] || [];
        if (absent.includes(c.itemCode)) return 0;

        // 승계 열은 매달 1일에만 값이 생깁니다 — 나머지 날은 applyCarry 가 채웁니다
        if (c.carry) {
            if (dateStr.slice(-2) !== '01') return undefined;
            return 6 + (sampleHash(dateStr + c.itemCode) % 2);   // 6 또는 7
        }

        const h = sampleHash(floorKey + dateStr + c.itemCode);
        if (h % 11 === 0) return 0;                              // 가끔 비는 칸
        return 20 + (h % 180);                                   // 20 ~ 199 롤
    }

    /* 조회 구간보다 앞에서 마지막으로 값이 생긴 날 — 임시 숫자에서는 '직전 달 1일'입니다.
       실제 DB 에서는 carry 열마다 "이 날짜 이전 마지막 1건" 조회가 이 자리에 옵니다. */
    function fillSampleCarrySeed(from) {
        App.COLUMNS.filter(c => c.carry).forEach(c => {
            const d = new Date(from + 'T00:00:00');
            d.setDate(1);
            // from 이 그달 1일이면 그날 값은 구간 안에 있으므로 한 달 더 거슬러 올라갑니다
            if (from.slice(-2) === '01') d.setMonth(d.getMonth() - 1);
            const seedDate = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-01`;

            FLOORS.forEach(f => {
                const v = sampleRoll(f.key, seedDate, c);
                if (v === undefined || v === 0) return;

                /* 이 구간(from)보다 앞선 씨앗이 이미 있으면 그대로 씁니다.
                   없으면 갈아 끼웁니다 — 과거로 스크롤해 더 이른 구간을 불러오면
                   기존 씨앗이 그 구간보다 뒤에 있어 쓸 수가 없기 때문입니다.
                   (뒤쪽 날짜는 조회 구간 안에 실제 입력값이 들어 있어 영향 없습니다) */
                const cur = state.carrySeed[f.key][c.itemCode];
                if (cur && cur.date < from) return;

                state.carrySeed[f.key][c.itemCode] = { date: seedDate, value: v };
            });
        });
    }

    function fillSample(from, to) {
        fillSampleCarrySeed(from);

        FLOORS.forEach(f => {
            for (let d = from; d <= to; d = addDays(d, 1)) {
                if (!state.values[f.key][d]) state.values[f.key][d] = {};
                if (!state.snapshot[f.key][d]) state.snapshot[f.key][d] = {};

                App.COLUMNS.forEach(c => {
                    const v = sampleRoll(f.key, d, c);
                    if (v === undefined) return;
                    state.values[f.key][d][c.itemCode] = v;
                    state.snapshot[f.key][d][c.itemCode] = v;
                });
            }
        });
    }
    ...
    /* ────────────────────────────────────────────────────────────
       데이터 조회
       ────────────────────────────────────────────────────────────
       DB 가 붙으면 이 함수의 몸통만 factory1_jigo_api.js 로 옮깁니다.
       읽어야 할 곳이 둘입니다.
         · 1~9열  : 이 페이지가 입력하는 테이블 (App.TABLE, 아직 미정)
         · 10~12열: factory1_ft_jigo — 'FT 일지'가 입력한 값을 읽기만 합니다
       ──────────────────────────────────────────────────────────── */
    App.fetchRange = async function (from, to) {
        ...
        // DB 연결 전 — 조회할 곳이 없어 표는 전부 '–' 로 남습니다.
    };
