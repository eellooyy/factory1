...
(function () {
    ...
    /* 급지 재고 뷰 — 저장하지 않고 좌측 값에서 매번 계산됩니다.
       이 페이지는 '전일 재고'(실사용량 계산용)를 읽을 때만 씁니다. 오늘 재고는
    ...
    App.state = {
        ...
        /* 전일 재고 (실사용량 계산용)
           실사용량 = 전일 재고 + 출고롤 × 중량 − 오늘 재고 입니다.

           [날짜 기준] 재고는 아침에 세지만 그 숫자는 '어제 작업이 끝난 뒤 남은 롤'
           이라 어제 자로 적습니다(지고 재고의 BASE_OFFSET_DAYS 와 같은 약속입니다).
           그래서 재고(D)는 D일 마감 재고이고, 출고(D) 역시 지고 재고를
           전일−금일로 뺀 값이라 D일 하루치입니다. 실사용량도 같은 구간이어야
           ERP 사용량(print_date = D)과 나란히 놓을 수 있습니다.

           예전에는 '오늘 재고 − 다음날 재고'로 계산했습니다. 재고를 당일 오전
           기준으로 적던 시절의 식이라 지금 기준에서는 하루씩 밀립니다. 게다가
           화면 기준일이 어제여서 '다음날(오늘) 재고'는 아직 세기 전이라 비어
           있었고, 실사용량이 재고 전액만큼 부풀었습니다.

           저장하지 않고 매번 뷰에서 읽습니다 — 이미 확정된 과거값이라 화면에서
           고칠 일이 없고, 편집으로 움직이는 건 뺄셈 쪽의 오늘 재고뿐입니다. */
        prevDayInventory: {},
