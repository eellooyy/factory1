...
(function () {
    ...
    window.Factory1InventoryIo = {
        ...
        /* 전자신문 대장 대조 ────────────────────────────────────────
           거래처(전자신문)가 관리하는 원장과 맞춰 보는 기능입니다. 아래
           LOCATIONS 에서 ledger: true 가 붙은 저장위치에만 배지가 생깁니다.

           ※ 이 대조는 '차이가 0 이 되어야 하는' 대조가 아닙니다.
             대장 출고 = 정미사용량 × 1.03 (약정 손지 3%)
             ERP  출고 = 급지 실사용량 (실손지 5~6%)
           라서 매일 200~600kg 씩 구조적으로 벌어집니다. 그래서 차이를
           '차이'가 아니라 '미정산 손지 누계'로 부릅니다. 0 을 기대하는
           이름을 붙이면 매일 오류로 읽히고 결국 아무도 안 봅니다.

           dailyDrift 는 그 정상 증가폭(하루 기준)입니다. 일별 대조가
           가능해지면(= 뷰가 붙으면) '어제 대비 증가분이 이 폭을 벗어난 날'만
           주황으로 표시할 자리입니다. 지금은 화면 기준일 대장이 있는지
           없는지만 보고 배지 상태를 정합니다. */
        LEDGER: {
            title: '전자신문 용지재고대장',
            pageUrl: 'factory1_jeonja.html',
            dailyDrift: 250,
            driftTolerance: 200
        },
        ...
        LOCATIONS: [
            ...
            },
            {
                ...
                locCode: 'WB11102',
                /* 거래처 원장이 있는 유일한 저장위치입니다. 이 줄이 있으면
                   저장위치 칸에 대장 배지가 붙고 대조 팝업이 열립니다.
                   지우면 배지와 팝업이 같이 사라집니다. */
                ledger: true,
                items: [
