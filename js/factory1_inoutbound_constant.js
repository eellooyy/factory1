/* factory1_inoutbound_constant.js — 1공장 입출고 내역 상수
   ...
   [입고는 층을 나누지 않습니다]
     ...
     화면에서는 한 표로 합칩니다. 대부분이 6층(B6)이라 우측 요약 칸의 제목
     자리에 '6층 입고' 태그를 세워 두고, 5층(B5)에서 들어온 것만 표의
     배경색과 요약 아래 '5층 입고' 줄로 따로 드러냅니다.
...
(function () {
    ...
    window.Factory1Inoutbound = {
        ...
        /* ── 요약 칸 제목 태그 ───────────────────────────────────
           우측 요약 칸의 제목 자리에 서는 태그입니다. '요약' 대신 '6층 입고'
           라고 붙여, 아래 품목 줄이 어느 층 것인지를 제목이 말하게 합니다.
           롤 수·Kg 은 붙이지 않습니다 — 제목이지 집계 줄이 아닙니다.

           label 뒤에는 보고 있는 방향이 붙습니다 (6층 입고 / 6층 출고).
           출고를 보면서 '입고'라고 적혀 있으면 안 되기 때문입니다.

           ※ 품목 줄은 5층·6층을 합친 숫자입니다. 5층이 들어온 날은 제목이
             가리키는 것보다 조금 큽니다. 그 몫은 아래 '5층 입고' 줄에
             그대로 나오므로 빼서 보면 됩니다.
             (2026-06-30 이후 B5 스캔은 한 건도 없습니다)
           ──────────────────────────────────────────────────────── */
        SIDE_TAG: { label: '6층', cls: 'f1iob-tag-b6' },

        /* ── 층별 태그 ───────────────────────────────────────────
           요약 아래에 층별 롤 수를 따로 세웁니다. 품목 줄과 성격이 다른
           숫자(어느 층으로 들어왔나)라 목록 밖에 둡니다.
           출고는 B6 하나뿐이라 이 줄이 합계와 같아지므로 입고에서만 나옵니다.

           tint: true 인 층만 상세 표에서 배경색으로도 구분합니다. B6 는
           입고의 대부분이라 색을 칠하면 표가 통째로 물들어 구분이 되지
           않습니다 — 6층은 위 제목 태그가 대신합니다.
           ──────────────────────────────────────────────────────── */
        FLOOR_TAGS: [
            { floor: 'B5', label: '5층 입고', cls: 'f1iob-tag-b5', tint: true }
        ],
