/* factory1_inoutbound_main.js — 1공장 입출고 내역 진입점
   ...
   기준일은 '오늘'로 시작합니다(공통 헤더 기본값은 어제라 진입 시 당겨 옵니다).
   이 화면의 숫자는 '어제 작업의 결과'가 아니라 '그 날짜에 실제로 찍힌 스캔'
   이므로, 오늘 찍힌 것부터 바로 보이는 편이 맞습니다.
...
(function () {
    ...
    const Factory1InoutboundModule = {
        init: function () {
            ...
            // 공통 헤더는 기본값이 '어제'이지만 이 화면은 '오늘'로 시작합니다.
            // (false = 데이터 재조회 없이 표시만 갱신 → 아래에서 한 번만 로드)
            // bindDayNav 보다 앞에서 당겨 둡니다. 그래야 표 우측의 '다음날' 띠가
            // 처음부터 오늘 기준으로 잠깁니다.
            App.headerApi.setCurrentDate(utils.getTodayStr(), false);
