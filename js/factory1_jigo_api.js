...
(function () {
    ...
    /* 저장 대상은 readonly 가 아닌 열입니다. 지금은 col 1~9 의 9품목(F.T 3종만
       읽기 전용)이고, 입력 주인이 바뀌면 상수의 readonly 만 고치면 여기가 따라옵니다. */
    ...
    /* ── 승계 씨앗 ────────────────────────────────────────────────────────────
       조회 구간(from)보다 앞에서 마지막으로 입력된 1건입니다. 승계 열은 값이
       바뀐 날만 적으므로, 15일치를 불러오는 것만으로는 직전 값을 못 보는 날이
       대부분이고 그러면 한 달 내내 빈칸이 됩니다.

       ※ 지금 이 페이지에는 carry 열이 하나도 없습니다(48.8g 이 매일 직접 입력으로
         바뀌면서 carry 를 뗐습니다). 그래서 이 함수는 첫 줄에서 빠져나갑니다.
         나라사랑 페이지의 급지대 잔량이 같은 구조를 쓸 예정이라 남겨 둡니다.
       ──────────────────────────────────────────────────────────────────────── */
